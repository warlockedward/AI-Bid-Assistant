...
# 延迟导入autogen，避免静态分析工具报错
def _import_assistant_agent():
    """延迟导入AssistantAgent"""
    try:
        agentchat = importlib.import_module("autogen.agentchat")
        return getattr(agentchat, "AssistantAgent")
    except (ImportError, AttributeError):
        # 在开发环境中可能没有安装autogen，使用模拟类
        class MockAssistantAgent:  # pylint: disable=too-few-public-methods
            """模拟AssistantAgent类用于开发环境"""
            def __init__(self, *args, **kwargs):
                pass
        return MockAssistantAgent


# 获取AssistantAgent类（用于类型检查和运行时）
AssistantAgentClass = _import_assistant_agent()


class ContentGenerationAgent(AssistantAgentClass):  # type: ignore
...
请确保生成的内容专业、准确、有说服力，能够有效展示投标优势。"""
        ...
        self.register_function(
            function_map={
                ...
                "review_and_refine_content":
                    self.review_and_refine_content
            }
        )

    def generate_technical_proposal(
        self,
        requirements: Dict[str, Any],  # pylint: disable=unused-argument
        knowledge: Dict[str, Any]  # pylint: disable=unused-argument
    ) -> Dict[str, Any]:
        """生成技术方案"""
        try:
            # 基于需求和知识生成技术方案
    ...
    def generate_commercial_proposal(
        self,
        requirements: Dict[str, Any],  # pylint: disable=unused-argument
        analysis: Dict[str, Any]  # pylint: disable=unused-argument
    ) -> Dict[str, Any]:
        """生成商务方案"""
        commercial_content = {
    ...
    def generate_implementation_plan(
        self,
        requirements: Dict[str, Any],  # pylint: disable=unused-argument
        risks: Dict[str, Any]  # pylint: disable=unused-argument
    ) -> Dict[str, Any]:
        """生成实施计划"""
    ...
    def generate_executive_summary(
        self,
        all_content: Dict[str, Any]  # pylint: disable=unused-argument
    ) -> str:
        """生成执行摘要"""
        summary = """
# 投标方案执行摘要
