    ...
    try:
        ...
        if not admin_user:
            ...
            admin_user = User(
                ...
                preferences={
                    "role": "system_admin",
                    "permissions": ["manage_tenants", "manage_users", "view_all_data"]
                }
            )
            db.add(admin_user)
        ...
        # 检查是否已存在租户配置
        tenant_config = db.query(TenantConfig).filter(TenantConfig.tenant_id == default_tenant.id).first()
...
def create_demo_tenant():
    ...
    try:
        ...
        # 创建演示租户配置
        demo_config = db.query(TenantConfig).filter(TenantConfig.tenant_id == demo_tenant.id).first()
