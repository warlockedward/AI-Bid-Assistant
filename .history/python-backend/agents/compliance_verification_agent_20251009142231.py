...
请确保验证全面、准确，为最终投标质量提供保障。"""
    ...
    async def _execute_impl(
        ...
        verification_results: Dict[str, Any] = input_data.get(
    ...
    def check_requirement_coverage(
        ...
        total_reqs = coverage_analysis["total_requirements"]
        coverage_analysis["coverage_rate"] = \
            float(covered_count) / max(1, float(total_reqs))
    ...
    def assess_overall_quality(
        ...
        # 技术质量评分
        tech_coverage: Dict[str, Any] = compliance_results.get(
            "technical_coverage", {})
        tech_score: float = self._calculate_technical_score(tech_coverage)
        quality_assessment["technical_score"] = tech_score
        
        # 商务质量评分
        commercial_coverage: Dict[str, Any] = compliance_results.get(
            "commercial_coverage", {})
        commercial_score: float = self._calculate_commercial_score(
            commercial_coverage)
        quality_assessment["commercial_score"] = commercial_score
        
        # 合规性评分
        compliance_score: float = self._calculate_compliance_score(
            compliance_results)
...
"""
    ...
    def _calculate_compliance_score(
        ...
        """计算合规评分"""
        technical_issues: int = len(
            compliance_results.get("technical_issues", []))
        commercial_issues: int = len(
            compliance_results.get("commercial_issues", []))
    ...
    def _generate_recommendations(
        ...
        # 基于商务问题生成建议
        commercial_issues: List[str] = compliance_results.get(
            "commercial_issues", [])
    ...
    def _format_compliance_details(
        ...
        tech_coverage: Dict[str, Any] = verification_results.get(
            "technical_coverage", {})
        if tech_coverage:
            rate: float = tech_coverage.get('coverage_rate', 0) * 100
            details += f"技术需求覆盖度: {rate:.1f}%\n"
        
        commercial_coverage: Dict[str, Any] = verification_results.get(
            "commercial_coverage", {})
    ...
    def _format_recommendations(
        ...
        """格式化建议"""
        recommendations: List[str] = verification_results.get(
            "recommendations", [])
