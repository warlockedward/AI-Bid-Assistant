    ...
    async def _execute_impl(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if operation == "generate_technical_proposal":
                result = self.generate_technical_proposal(requirements, knowledge)
            elif operation == "generate_commercial_proposal":
                result = self.generate_commercial_proposal(requirements, analysis)
            elif operation == "generate_implementation_plan":
                result = self.generate_implementation_plan(requirements, risks)
            elif operation == "generate_executive_summary":
                result = {"summary": self.generate_executive_summary(all_content)}
            elif operation == "review_and_refine_content":
                result = self.review_and_refine_content(all_content, feedback)
            else:
                # 默认操作
                result = self.generate_technical_proposal(requirements, knowledge)
            ...
            raise RuntimeError(f"Content generation failed: {str(error)}") from error
