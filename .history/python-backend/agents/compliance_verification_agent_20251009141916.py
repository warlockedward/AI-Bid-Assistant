    ...
    async def _execute_impl(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if operation == "verify_technical_compliance":
                result = self.verify_technical_compliance(requirements, content)
            elif operation == "verify_commercial_compliance":
                result = self.verify_commercial_compliance(requirements, content)
            elif operation == "check_requirement_coverage":
                result = self.check_requirement_coverage(requirements, content)
            elif operation == "assess_overall_quality":
                result = self.assess_overall_quality(content, verification_results)
            elif operation == "generate_compliance_report":
                result = {"report": self.generate_compliance_report(verification_results)}
            else:
                # 默认操作
                result = self.verify_technical_compliance(requirements, content)
            ...
            raise RuntimeError(f"Compliance verification failed: {str(error)}") from error
