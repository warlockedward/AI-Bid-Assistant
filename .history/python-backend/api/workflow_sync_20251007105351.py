...
async def sync_workflow_state(
    ...
    try:
        ...
        error_msg = f"Failed to sync workflow state: {str(e)}"
...
async def get_workflow_state(
    ...
    try:
        ...
        error_msg = f"Failed to get workflow state: {str(e)}"
        raise HTTPException(status_code=500, detail=error_msg)
...
async def get_checkpoints(
    ...
    try:
        ...
        checkpoints = repo.get_workflow_checkpoints(
            tenant_context, 
            workflow_uuid, 
            limit
        )
        ...
        error_msg = f"Failed to get checkpoints: {str(e)}"
        raise HTTPException(status_code=500, detail=error_msg)
    ...
    try:
        ...
        updated_state_data = {
            **workflow.state_data,
            ...
            f"{agent_result.step_id}_execution_time": (
        ...
        error_msg = f"Failed to sync agent result: {str(e)}"
        raise HTTPException(status_code=500, detail=error_msg)
...
async def report_workflow_error(
    ...
    try:
        ...
        error_msg = f"Failed to report workflow error: {str(e)}"
        raise HTTPException(status_code=500, detail=error_msg)
...
async def get_active_workflows(
    ...
    try:
        ...
        error_msg = f"Failed to get active workflows: {str(e)}"
        raise HTTPException(status_code=500, detail=error_msg)