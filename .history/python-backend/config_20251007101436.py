...
class Config:
    ...
    # 数据库配置
    # ===========================================
    DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://user:password@localhost/bid_system")
    ...
    OPENAI_CONFIG = {
        ...
        "available_models": os.getenv("OPENAI_AVAILABLE_MODELS", "gpt-4,gpt-4-turbo,gpt-3.5-turbo,gpt-4o").split(","),
        "temperature": 0.1,
        "timeout": 600,
        "max_tokens": 4000
    }
    ...
    VLLM_CONFIG = {
        ...
        "available_models": os.getenv("VLLM_AVAILABLE_MODELS", "llama-2-7b-chat,llama-2-13b-chat,codellama-7b-instruct").split(","),
        "temperature": 0.1,
        "timeout": 120,
        "max_tokens": 2000
    }
...
class ModelConfigManager:
    ...
    @staticmethod
    def get_model_config(provider: ModelProvider, model_name: Optional[str] = None) -> Dict[str, Any]:
        """获取指定提供商和模型的配置"""
    ...
    @staticmethod
    def get_autogen_config(provider: ModelProvider, model_name: Optional[str] = None, tenant_id: Optional[str] = None) -> Dict[str, Any]:
        """获取AutoGen格式的配置"""
        model_config = ModelConfigManager.get_model_config(provider, model_name)
    ...
    def get_available_models() -> Dict[str, List[str]]:
        """获取所有可用的模型列表"""
        return {
            "openai": Config.OPENAI_CONFIG["available_models"],
            "vllm": Config.VLLM_CONFIG["available_models"]
        }

def get_tenant_config(tenant_id: str, provider: ModelProvider = ModelProvider.OPENAI, model_name: Optional[str] = None) -> Dict[str, Any]:
    """获取租户特定配置"""
    return ModelConfigManager.get_autogen_config(provider, model_name, tenant_id)
...
def validate_config() -> List[str]:
    ...
    # 验证安全配置
    if not Config.SECURITY_CONFIG["jwt_secret"] or Config.SECURITY_CONFIG["jwt_secret"] == "your-secret-key-change-in-production":
