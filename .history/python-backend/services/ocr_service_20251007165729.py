...
class OCRService:
    ...
    def __init__(self):
        ...
        if self.default_engine not in self.available_engines:
            self.default_engine = self.available_engines[0] if self.available_engines else "marker-pdf"
    ...
    async def process_document(
        ...
        if not self.is_engine_available(engine_to_use):
            return {
                ...
                "processing_time": 0,
                "accuracy": 0,
                "error": f"OCR引擎 '{engine_to_use}' 不可用"
            }
        
        try:
            # 根据不同引擎调用相应的处理逻辑
            if engine_to_use == OCREngine.MINERU.value:
                result = await self._process_with_mineru(file_data, file_name, mime_type)
            elif engine_to_use == OCREngine.MARKER_PDF.value:
                result = await self._process_with_marker_pdf(file_data, file_name, mime_type)
            elif engine_to_use == OCREngine.OLMOCR.value:
                result = await self._process_with_olmocr(file_data, file_name, mime_type)
            else:
                result = await self._process_with_marker_pdf(file_data, file_name, mime_type)
            ...
            logger.info(f"OCR处理完成 - 引擎: {engine_to_use}, 文件: {file_name}, 耗时: {processing_time:.2f}秒")
            return result
            
        except Exception as e:
            processing_time = asyncio.get_event_loop().time() - start_time
            logger.error(f"OCR处理失败 - 引擎: {engine_to_use}, 文件: {file_name}, 错误: {str(e)}")
            return {
                "success": False,
                "content": "",
                "engine_used": engine_to_use,
    ...
    async def _process_with_mineru(
        ...
        Returns:
            Dict: OCR处理结果
        """
        logger.info(f"使用MinerU处理文档: {file_name}")
        ...
        try:
            ...
            return {
                "success": True,
                "content": f"这是使用MinerU OCR引擎处理的{file_name}文档内容。\n\n文档包含多页内容，MinerU能够准确识别复杂布局和表格结构。",
                "engine_used": OCREngine.MINERU.value,
                "accuracy": 95
            }
        except Exception as e:
            raise Exception(f"MinerU处理失败: {str(e)}")
    ...
    async def _process_with_marker_pdf(
        ...
        Returns:
            Dict: OCR处理结果
        """
        logger.info(f"使用Marker PDF处理文档: {file_name}")
        ...
        try:
            ...
            return {
                "success": True,
                "content": f"这是使用Marker PDF OCR引擎处理的{file_name}文档内容。\n\n该引擎快速高效，适用于大多数PDF文档类型。",
                "engine_used": OCREngine.MARKER_PDF.value,
                "accuracy": 92
            }
        except Exception as e:
            raise Exception(f"Marker PDF处理失败: {str(e)}")
    ...
    async def _process_with_olmocr(
        ...
        Returns:
            Dict: OCR处理结果
        """
        logger.info(f"使用OLM OCR处理文档: {file_name}")
        ...
        try:
            ...
            return {
                "success": True,
                "content": f"这是使用OLM OCR引擎处理的{file_name}文档内容。\n\nOLM OCR支持多语言识别和手写体识别，具有最高的识别准确率。",
                "engine_used": OCREngine.OLMOCR.value,
                "accuracy": 97
            }
        except Exception as e:
            raise Exception(f"OLM OCR处理失败: {str(e)}")
