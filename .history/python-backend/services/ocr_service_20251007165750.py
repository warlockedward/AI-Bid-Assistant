...
class OCRService:
    ...
    async def process_document(
        ...
        try:
            ...
            logger.info(
                f"OCR处理完成 - 引擎: {engine_to_use}, "
                f"文件: {file_name}, 耗时: {processing_time:.2f}秒"
            )
            return result
            ...
            processing_time = asyncio.get_event_loop().time() - start_time
            logger.error(
                f"OCR处理失败 - 引擎: {engine_to_use}, "
                f"文件: {file_name}, 错误: {str(e)}"
            )
            return {
                "success": False,
                "content": "",
    ...
    async def _process_with_mineru(
        ...
        try:
            ...
            return {
                "success": True,
                "content": (
                    f"这是使用MinerU OCR引擎处理的{file_name}文档内容。\n\n"
    ...
    async def _process_with_marker_pdf(
        ...
        try:
            ...
            return {
                "success": True,
                "content": (
                    f"这是使用Marker PDF OCR引擎处理的{file_name}文档内容。\n\n"
    ...
    async def _process_with_olmocr(
        ...
        try:
            ...
            return {
                "success": True,
                "content": (
                    f"这是使用OLM OCR引擎处理的{file_name}文档内容。\n\n"
