...
class OCRService:
    ...
    async def _process_with_mineru(
        self,
        file_data: bytes,  # pylint: disable=unused-argument
        file_name: str,
        mime_type: str  # pylint: disable=unused-argument
    ) -> Dict[str, Any]:
        """
        使用MinerU处理文档
        ...
        try:
            ...
            raise RuntimeError("MinerU处理失败: %s" % str(e)) from e
    ...
    async def _process_with_marker_pdf(
        ...
        try:
            ...
            raise RuntimeError("Marker PDF处理失败: %s" % str(e)) from e
    ...
    async def _process_with_olmocr(
        ...
        try:
            ...
            raise RuntimeError("OLM OCR处理失败: %s" % str(e)) from e
