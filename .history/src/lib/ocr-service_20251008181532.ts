...
class OCRService {
  ...
  constructor() {
    ...
    this.testMode = process.env.NODE_ENV === 'development' && process.env.OCR_TEST_MODE === 'true';
    // 默认配置
    this.config = {
  ...
  async processDocument(options: OCRProcessingOptions): Promise<OCRResult> {
    // 如果是测试模式，使用模拟数据
    if (this.testMode) {
      return this.processDocumentMock(options);
    }
   ...
   * 模拟OCR处理（测试模式）
   */
  private async processDocumentMock(options: OCRProcessingOptions): Promise<OCRResult> {
    const engine = options.engine || this.config.defaultEngine;
    const startTime = Date.now();
    ...
    const accuracy = Math.floor(90 + Math.random() * 10); // 90-100%准确率

    if (success) {
      return {
        success: true,
        content: `这是使用${engine}引擎处理的${options.fileName}文件内容。\n\nOCR处理成功，识别准确率为${accuracy}%。`,
        engineUsed: engine,
        processingTime: Date.now() - startTime,
        accuracy: accuracy
      };
