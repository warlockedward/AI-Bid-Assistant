  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        {/* 浮动粒子 - 高科技感 */}
        <div className="absolute inset-0">
          {typeof window !== 'undefined' && [...Array(80)].map((_, i) => (
            <div
              key={i}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-8 py-24">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 mb-24">
          {stats.map((stat, index) => (
            <div key={stat.label} className="text-center float-3d">
              <div className="relative mb-6">
                <div className={`w-20 h-20 bg-gradient-to-r from-primary to-secondary rounded-2xl flex items-center justify-center mx-auto shadow-xl ${stat.animation}`}>
                  <stat.icon className="h-10 w-10 text-white" />
                </div>
                <div className="absolute -top-3 -right-3 w-6 h-6 bg-gradient-to-r from-yellow-400 to-orange-500 rounded-full animate-pulse shadow-md"></div>
              </div>
              <div className="text-4xl font-bold text-white mb-3 gradient-text">{stat.value}</div>
        ...
        <div className="mb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
            {features.map((feature, index) => (
              <div key={feature.title} className={`relative group ${feature.animation}`}>
                <div className={`absolute inset-0 bg-gradient-to-r ${feature.color.replace('to-', 'to-').replace('from-', 'from-').split(' ').map(c => c + '/15').join(' ')} rounded-[2rem] blur-lg group-hover:blur-xl transition-all duration-500`}></div>
                
                <div className="relative pixar-card bg-white/10 border-2 border-white/20 rounded-[2rem] p-8 backdrop-blur-sm group-hover:bg-white/15 transition-all duration-500">
                  <div className={`w-16 h-16 bg-gradient-to-r ${feature.color} rounded-2xl flex items-center justify-center mb-6 shadow-xl tech-glow`}>
                  ...
                  <div className="absolute top-3 right-3 w-3 h-3 bg-gradient-to-r from-yellow-400 to-orange-500 rounded-full animate-pulse"></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="text-center mb-32 relative">
          ...
          <div className="pixar-card p-12 rounded-3xl border-2 border-white/20 backdrop-blur-sm">
            <h2 className="text-5xl font-bold mb-6">
              <span className="gradient-text tech-glow">准备好提升您的投标效率了吗？</span>
            </h2>
            <p className="text-2xl text-gray-200 mb-10 max-w-3xl mx-auto">
              立即开始使用AI Bid Assistant，让AI为您的投标业务赋能
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-6">
              <Link href="/register" className="pixar-button tech-border bg-gradient-to-r from-primary to-secondary text-white px-10 py-4 rounded-full text-xl font-bold hover:shadow-2xl transition-all duration-500 transform hover:scale-105">
                免费试用
              </Link>
              <Link href="/contact" className="pixar-button bg-white/10 text-white px-10 py-4 rounded-full text-xl font-bold hover:bg-white/20 transition-all duration-500 border-2 border-white/30 transform hover:scale-105">
                联系我们
              </Link>
            </div>
          </div>
