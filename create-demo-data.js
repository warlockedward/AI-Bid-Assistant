...
async function main() {
  const prisma = new PrismaClient({
    datasources: {
      db: {
        url: 'postgresql://postgres:password@localhost:5432/intelligent_bid_system'
      }
    }
  });
  ...
  try {
    ...
    if (!demoTenant) {
      ...
      await prisma.tenantConfig.create({
        data: {
          ...
          ragApiUrl: 'https://api.example.com/rag',
      ...
      console.log('演示租户已存在:', demoTenant.name);
    }
    ...
    if (!demoUser) {
      ...
      console.log('演示用户已存在:', demoUser.name);
    }
